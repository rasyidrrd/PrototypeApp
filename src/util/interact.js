...
export const loadCurrentMessage = async () => { 
  const message = await TestContractContract.methods.message().call(); 
  return message;
};
...
export const resetMetamaskConnection = async () => {
  try {
    if (window.ethereum && window.ethereum.selectedAddress) {
      ...
      // Notify the user to reconnect manually
      return { status: "🔄 Metamask connection disconnected. Please reconnect manually." };
    }

    return { status: "🔄 Metamask connection was already disconnected." };
  } catch (error) {
    return { status: "😥 Error resetting Metamask connection: " + error.message };
  }
};
